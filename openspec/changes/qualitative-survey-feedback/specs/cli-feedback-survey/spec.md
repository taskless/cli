...
- **AND** PostHog SHALL receive `survey shown` with `topic: "create-sg-rule"`
...
- **THEN** both SHALL append the invite and capture `survey shown` with `topic: "onboard"`
...
### Requirement: The cadence store is one epoch timestamp per CLI version

The CLI SHALL keep the survey cadence at `<config directory>/surveys/<cli version>/next_ask`, where the config directory is the same XDG location that holds the anonymous telemetry id. The file SHALL contain a single decimal epoch-milliseconds value: the earliest time the next invite may be served. Serving an invite SHALL set it to now plus 10 days; `feedback dismiss` and `feedback send` SHALL each set it to now plus 20 days. A different CLI version SHALL read and write its own file.

#### Scenario: A new CLI version asks again

- **WHEN** version A served an invite yesterday and the user runs version B for the first time
- **THEN** version B SHALL find no `next_ask` of its own and SHALL serve the invite
...
- **WHEN** an agent runs `taskless feedback dismiss --topic create-sg-rule`
- **THEN** PostHog SHALL receive `survey dismissed` with `$survey_id` and `topic: "create-sg-rule"`
...
- **THEN** PostHog SHALL receive `survey sent` carrying `$survey_id`, `$survey_questions`, one `$survey_response_<question id>` per answered question, and `topic`
...
| key                | required | value                                                                        |
| ------------------ | -------- | ---------------------------------------------------------------------------- |
| `topic`            | yes      | one of `onboard`, `create-sg-rule`, `create-vale-rule`, `create-remote-rule` |
...
#### Scenario: Topic is validated

- **WHEN** a payload has `topic: "check"`
- **THEN** validation SHALL fail naming `topic`
...
The CLI SHALL embed a `feedback-invite` recipe carrying the text appended to surveyed recipes. Rendered header-less, it SHALL instruct the agent to ask the user exactly once, with the sentence "Taskless would like to know how the CLI is doing. Would you be okay sharing a few sentences about your experience? Or just skip it with `skip`."; to treat a reply of `skip`, silence, or a reply unrelated to feedback as a dismissal and run `feedback dismiss`; and to treat any other reply as feedback and fetch `agent feedback`. It SHALL name the topic it was attached to so the agent carries it into the payload and the dismissal. Both recipes SHALL follow the recipe conventions: a `# Topic:` header, the CLI named by its rendered invocation, and commands that exist.
...
- **AND** SHALL name the served topic and the rendered CLI invocation for both `feedback dismiss` and `agent feedback`
