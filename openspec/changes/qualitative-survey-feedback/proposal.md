...
- **A survey invite is appended to four served recipes** (`onboard`, `create-sg-rule`, `create-vale-rule`, `create-remote-rule`) when telemetry is enabled, the run is not in CI, and the per-CLI-version `next_ask` timestamp under the XDG config directory has passed. Serving the invite captures `survey shown` and pushes `next_ask` out 10 days. Both serving paths for onboarding (`taskless onboard` and `taskless agent onboard`) carry the invite.
...
- **The three survey events carry `topic`** so the shown → dismissed/sent funnel can be split per recipe, and carry the standard super-properties like every other capture, so `cliVersion` splits it per release. Event names and `$survey_*` keys are PostHog's contract and are used as-is; they are the one exception to the `cli_` prefix rule.
...
- PostHog: three new event names in the project, each carrying `topic` and the standard super-properties. No dashboard depends on them yet.
