...
- [x] 7.7 Archive the change

**Archive ordering, for the record.** `agent-command-and-vale-authoring` had to archive first: a change archives once on the tip and the gate wants `openspec/changes/` empty, and this change's `cli-agent-authoring` delta modifies a requirement that one introduces, so it needed a target in `specs/`. Both archives land on the tip PR.
