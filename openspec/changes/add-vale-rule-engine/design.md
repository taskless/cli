...
Exporting the topic through `@taskless/cli/prompts` was deliberately **not** coupled to this change: the export mechanism is a separate change, and the topic file is useful to the local `help` flow with or without it. That ordering has since resolved — `export-knowledge-prompts` landed first, so the one-line entry falls here.

It goes in `TOPICS`, not `INTERNAL_TOPICS`, and the distinction is functional rather than a label. `TOPICS` generates the exported `PromptTopic` union, the argument `getPrompt()` accepts, and the keys of the published `PROMPTS` record — so an entry there is what makes the topic renderable by a consumer outside this package. `INTERNAL_TOPICS` has no runtime effect at all; it is a ledger that lets the completeness check account for a recipe deliberately withheld. Neither list affects `taskless help`, which resolves recipes directly.

The consumer is the reason. This topic exists because engine selection currently lives in the platform generator as a binary `static | runtime` classifier that predates Vale — that is, a second copy of this decision, now wrong. Exporting the entry is what lets the generator render the same text `help` serves rather than maintaining its own; withholding it would leave the two free to drift, which is the failure the prompts module exists to prevent. The cost accepted in exchange: an exported name is a promise held for a major version, so removing it later is a breaking change.
