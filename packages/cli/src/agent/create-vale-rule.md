# Topic: create-vale-rule (CLI v%(CLI_VERSION)s / topic v4)
...
1. **Say what the rule reads, then pick an extension point.** Write one
   sentence: "this fires when a document contains \_\_\_." Vale rules are
...
| `scope`              | reaches                                                  |
| -------------------- | -------------------------------------------------------- |
| _(omitted)_          | everything the format exposes as prose                   |
...
`<figcaption>` nested in `<figure>` is invisible to _every_ scope,
...
`<!-- vale <id>.<id> = NO -->` directive is applied to the _parsed_
...
"Missing the required 'extends' key". Their _values_ are case-sensitive
...
_replacement_, not the match, so `"Use GitHub not %%s"` against the text
...
3. **Know what you are writing: `tokens` and `swap` keys are patterns,
   ...
   _This step is about `tokens` and `swap` only. A `capitalization`,
   `occurrence` or `metric` rule has neither, skip to step 4._
   - `(?:…)`, `[…]`, `|`, `+`, `?` all work.
...
- **URLs and code spans are not prose.** A `Github` key fires on
  ...
  `` `Github/docs` ``.
- **Link text _is_ prose.** In `[click here](https://example.com)`,
...
4.  **Scope the rule** by writing `.taskless/rules/vale/<id>/.vale.ini`.
    ...
    **Scope a rule _out_ with a second matcher, not a cleverer glob.** A
...
5.  **Write the fixtures.** Two directories inside the rule, both flat.
    ...
    **The `pass/` bucket is not "correct prose".** Correct prose proves
    ...
    - **`scope`d rules**: the same phrase _outside_ the scope. A rule
...
with punctuation is a _pattern_: `maybe?` means "mayb" plus an optional
...
check tests a _scope_, not a word. Note also that `ignorecase: true` is
...
Whenever a rule is about a _place_ in the document, the `pass/` fixture
...
**Goes wrong:** expecting it to pick a winner. It flags the _second_
...
- Vale reads one document at a time and has no cross-document view. A
  rule about consistency _between_ documents cannot be written here.
